...
public record Town(String name, String island, int level,
    ...
    ///compact constructor to initialize the loot, features, and opponents
    public Town {
        loot = randomReduced(new ArrayList<>(EnumSet.allOf(Loot.class)),
                level + 2); // first level has 2 treasures, second has 3, etc.
    ...
    //randomizes a list and returns a sublist of those elements. no defensive copy, because a new list will be passed to each call
    ...
    /// creating own accessor methods so it returns defensive copies of each.
